...
<body>
    ...
    echo '<div class="h-screen cursor-context-menu border b-gray-400 rounded flex flex-col justify-center items-center text-center p-6 bg-white">';
    echo '
        <div class="text-md font-bold flex flex-col text-gray-900">
            <span class="uppercase">'. $xml->nombre .'</span> 
            <span class="font-normal mt-4 text-gray-700 text-sm">'.$xml->elaborado.'</span>
        </div>
    ...
    <p class="text-gray-700 mb-2">Provincia: '.$xml->provincia.'</p>

    <div class="text-3xl font-bold text-gray-900 mb-6">'.$xml->prediccion->dia->temperatura->maxima.'º<span class="font-normal text-gray-700 mx-1">/</span>'.$xml->prediccion->dia->temperatura->minima.'º</div>
    
    <p class="text-gray-700 mb-2">Humedad</p>

    <div class="text-1xl font-bold text-gray-900 mb-6">'.$xml->prediccion->dia->humedad_relativa->maxima.'%<span class="font-normal text-gray-700 mx-1">/</span>'.$xml->prediccion->dia->humedad_relativa->minima.'%</div>
    
    <div class="flex justify-center w-full">
        <div class="flex items-center text-gray-700 px-2">
            velocidad del viento: '.$xml->prediccion->dia->viento->direccion[1].' km/h
        </div>

        <div class="flex items-center text-gray-700 px-2">
            direccion del viento: '.$xml->prediccion->dia->viento->velocidad[1].' km/h
        </div>

    </div>

    <button><a href="index.php" class="mt-12 inline-block px-6 py-2.5 bg-purple-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-purple-700 hover:shadow-lg focus:bg-purple-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-purple-800 active:shadow-lg transition duration-150 ease-in-out">Volver atras</a></button>
